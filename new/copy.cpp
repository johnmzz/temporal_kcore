...
void Graph::time_range_kcore_parallel(long _ts, long _te, int k) {
    ...
    while (update) {
        ...
        for (int u = 1; u < n_; ++u) {
            vector<int> oldCT = ct_[u];

            local_ct(u, 0, te - ts, k);

            for (int i = 0; i < ct_[u].size(); i++) {
                if (oldCT[i] != ct_[u][i]) {
                    update = true;
                }
            }
        }
        // cout << "finished round " << round << endl;
        round++;
    }
